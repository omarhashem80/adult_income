...
def model():
    ...
    age = st.number_input("Age", min_value=19, max_value=df['age'].max(), step=1,key=f"Age")
    input_data['age'] = age
    ...
    input_data['education-num'] = df.loc[df['education'] == education, 'education-num'].iloc[0],

    occupation_options = update_occupation_options(input_data['age'], education, input_data['workclass'])
    occupation = st.selectbox("Occupation", occupation_options, key=f"Occupation")
    ...
    relationship_options = df[df['sex'] == input_data['sex']]['relationship'].unique()
    relationship = st.selectbox("Relationship", relationship_options, key=f"Relationship")
    input_data['relationship'] = relationship

    marital_status_options = update_marital_status_options(input_data['relationship'])
    marital_status = st.selectbox("Marital Status", marital_status_options, key=f"Marital_Status")
    ...
    hours_per_week = st.number_input("Hours per Week", min_value=df['hours-per-week'].min(), max_value=df['hours-per-week'].max(), step=1, key=f"Hours_per_Week")
    input_data['hours-per-week'] = hours_per_week

    native_country_options = df['native-country'].unique().tolist()
    native_country = st.selectbox("Native Country", native_country_options, key=f"Native_Country")
    input_data['native-country'] = native_country

    capital_gain = st.number_input("Capital Gain", min_value=0, max_value=100000, step=100, key=f"Capital_Gain")

    capital_loss = st.number_input("Capital Loss", min_value=0, max_value=100000, step=100, key=f"Capital_Loss")
    input_data['Net_Capital_Change'] = capital_gain- capital_loss

    fnlwgt = st.number_input("fnlwgt", min_value=0, max_value=100000, step=100, key=f"fnlwgt")
